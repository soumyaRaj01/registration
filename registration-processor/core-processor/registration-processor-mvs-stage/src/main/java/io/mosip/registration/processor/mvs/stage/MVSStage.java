package io.mosip.registration.processor.mvs.stage;
...
@ComponentScan(basePackages = { "${mosip.auth.adapter.impl.basepackage}",
		...
		"io.mosip.registration.processor.mvs.config",
		...
		"io.mosip.registration.processor.mvs.validators"})
public class MVSStage extends MosipVerticleAPIManager {

	private static final String STAGE_PROPERTY_PREFIX = "mosip.regproc.mvs.";
	private static final String CLASS_NAME = "MVSStage";
	...
	/** The reg proc logger. */
	private static Logger regProcLogger = RegProcessorLogger.getLogger(MVSStage.class);
	...
	@Autowired
	MVSRequestValidator mVSRequestValidator;

	@Autowired
    MVSExceptionHandler mVSExceptionHandler;
	
	@Autowired
	PriorityBasedPacketManagerService packetManagerService;
	...
	/** The username. */
	@Value("${registration.processor.mvs.queue.username}")
	private String username;

	/** The password. */
	@Value("${registration.processor.mvs.queue.password}")
	private String password;

	/** The type of queue. */
	@Value("${registration.processor.mvs.queue.typeOfQueue}")
	private String typeOfQueue;

	/** The address. */
	@Value("${registration.processor.mvs.queue.response:mvs-to-mosip}")
	private String mvResponseAddress;

	@Value("#{'${registration.processor.mvs.queue.trusted.packages}'.split(',')}")
	...
	/** The url. */
	@Value("${registration.processor.mvs.queue.url}")
	private String url;
	...
	@Value("${mosip.regproc.mvs.message.expiry-time-limit}")
...
//		process(message);
	...
	public void sendMessage(MessageDTO messageDTO) {
		this.send(this.mosipEventBus, MessageBusAddress.MVS_BUS_OUT, messageDTO);
	}

	@Override
	public MessageDTO process(MessageDTO object) {
		return mVSService.process(object, queue, CLASS_NAME);
	}
